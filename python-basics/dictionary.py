...
# You can also construct a dictionary using the built in function called dict()
...
# The interpreter raises the same exception, KeyError, when a dictionary is accessed with either an
# undefined key or by a numeric index
# In the latter case, [1] looks like a numerical index, but it isn’t
d = {0: 'a',1: 'b', 2: 'c', 3: 'd'}

# In the expressions d[1], d[0], and d[2], the numbers in square brackets appear as though
# they might be indices. But they have nothing to do with the order of the items in the dictionary.
...
# A dictionary maps each key to a corresponding value, so it doesn’t make sense to
# map a particular key more than once.
# Second, when you assign a value to an already existing dictionary key, it does not
# add the key a second time, but replaces the existing value:
# Third, if you specify a key a second time during the initial creation of a
# dictionary, the second occurrence will override the first
# Fourth, neither a list nor another dictionary can serve as a dictionary key,
# because lists and dictionaries are mutable

# Operators in dictionary
# The in and not in operators return True or False according to whether the
# specified operand occurs as a key in the dictionary
...
# items() -> returns a list of key-value pairs in a dictionary. locations.items()
# returns a list of tuples containing the key-value pairs in items.
locations.items()
...
# pop() raises a KeyError exception if <key> is not found.
...
# popitem -> popitem() removes the last key-value pair added from d and
# returns it as a tuple
locations.popitem()
# If locations is empty, locations.popitem() raises a KeyError exception

# update -> Merges a dictionary with another dictionary or with an iterable
# of key-value pairs.
...
# 2. If the key is already present in d, the corresponding value in d for that key
# is updated to the value from <obj>.
d1 = {'a': 10, 'b': 20, 'c': 30}
d2 = {'b': 200, 'd': 400}

d1.update(d2)
# In this example, key 'b' already exists in d1, so its value is updated to 200,
# the value for that key from d2. However, there is no key 'd' in d1, so that
# key-value pair is added from d2.
# <obj> may also be a sequence of key-value pairs, similar to when the dict()
# function is used to define a dictionary. For example, <obj> can be specified
# as a list of tuples
d1 = {'a': 10, 'b': 20, 'c': 30}
